              ...
              <table class="w-full bg-white shadow-md rounded-lg overflow-hidden">
                  <thead class="bg-gray-200 text-gray-700">
                      <tr>
                          ...
                          <th class="p-3">Event Status</th>
                      </tr>
                  </thead>
                  <tbody class="text-center">
                      @foreach ($eventlists as $index => $eventlist)
                          @php $count++; @endphp
                          <tr class="{{ $count % 2 == 0 ? 'bg-gray-100' : 'bg-white' }}">
                              <td class="p-3">{{ $count }}</td>
                              <td class="p-3">{{ $eventlist['userid'] }}</td>
                              <td class="p-3">{{ $eventlist['username'] }}</td>
                              <td class="p-3">{{ $eventlist['datetime'] }}</td>
                              <td class="p-3">{{ $eventlist['eventname'] }}</td>
                          </tr>
                      @endforeach
                  </tbody>
              </table>
